...
namespace MathSite.Facades.Posts
{
    public class PostsFacade : BaseFacade<IPostsRepository, Post>, IPostsFacade
    {
        ...
        public async Task<int> GetPostPagesCountAsync(string postTypeAlias, RemovedStateRequest state,
        ...
        public async Task<int> GetPostPagesCountAsync(string postTypeAlias, int perPage, RemovedStateRequest state,
            PublishStateRequest publishState, FrontPageStateRequest frontPageState, bool cache)
        {
        ...
        public async Task<Post> GetPostByUrlAndTypeAsync(Guid currentUserId, string url, string postTypeAlias,
            bool cache)
        {
        ...
        {
            return await GetPostsAsync(postTypeAlias, page, await GetPerPageCountAsync(cache),
                RemovedStateRequest.Excluded, PublishStateRequest.Published, FrontPageStateRequest.AllVisibilityStates,
                cache);
        }
            ...
            bool cache)
        {
            ...
            var cacheKey =
                $"Post={postTypeAlias}:Page={page}:PerPage={perPage}:Removed={state}:Published={publishState}:FrontPage={frontPageState}";
            ...
            return cache
                ? await MemoryCache.GetOrCreateAsync(
                    ...
                    async entry =>
                    {
                        ...
                        return await RepositoryManager.PostsRepository.GetAllWithAllDataIncludedPagedAsync(requirements,
                            perPage, toSkip);
                    })
                : await RepositoryManager.PostsRepository.GetAllWithAllDataIncludedPagedAsync(requirements, perPage,
                    toSkip);
        }
        ...
        private async Task<int> GetPostsWithTypeCount(string postTypeAlias, RemovedStateRequest state,
            PublishStateRequest publishState, FrontPageStateRequest frontPageState, bool cache)
        {
            var requirements = CreateRequirements(postTypeAlias, state, publishState, frontPageState);
