...
namespace MathSite.Domain.Logic.PostSettings
{
    public class PostSettingLogic : LogicBase<PostSetting>, IPostSettingLogic
    {
        ...
        public async Task<Guid> CreateAsync(bool isCommentsAllowed, bool canBeRated, bool postOnStartPage,
            Guid? previewImageId)
        {
            var id = Guid.Empty;
        ...
        public async Task UpdateAsync(Guid id, bool isCommentsAllowed, bool canBeRated, bool postOnStartPage,
            Guid? previewImageId)
        {
            await UseContextWithSaveAsync(async context =>
            {
